...
void main() {
    // ── Depth coordinate (positive = submerged, negative = exposed land) ──────
    // depth0: unperturbed, used for the foam system to keep foam placement stable.
    // depth:  wave-perturbed, used for all color transitions to break up the
    //         horizontal "band" look — every X position along the beach sees a
    //         slightly different depth phase, giving an organic irregular boundary.
    float depth0 = u_WaterlineZ - v_World.z;

    float waveH  = sin(v_World.z * 1.9  + v_World.x * 0.75 + u_Time * 0.45) * u_WindAmp * 1.1
                 + sin(v_World.z * 3.4  - v_World.x * 1.15 - u_Time * 0.62) * u_WindAmp * 0.55
                 + bN(v_World.xz * 0.22 + vec2(u_Time * 0.05, -u_Time * 0.03)) * u_WindAmp * 0.90;
    float depth  = depth0 + waveH;

    // distToWater: unperturbed, landward-positive (foam and swash use this)
    float distToWater = -depth0;

    // Discard anything deeper than 8 m (ocean mesh already covers those pixels)
    if (depth0 > 8.0) discard;

    // Camera distance for LOD and light corridor
    vec2  toFragXZ = v_World.xz - u_CamPos.xz;
    float dCam     = max(length(toFragXZ), 0.01);

    // ── Wetness: tied to perturbed depth, so wet/dry boundary is wave-irregular ─
    // depth =  0 (waterline) → fully wet;  depth = -10 (10 m exposed) → fully dry
    float wetness = clamp(1.0 + depth / 10.0, 0.0, 1.0);
    ...
    // Mudflat factor: only on exposed surface — fades to 0 as depth goes positive
    float mudflatFactor = u_MudflatExposure * smoothstep(1.5, -2.0, depth);

    // ── Base exposed-surface color ─────────────────────────────────────────────
    ...
    if (mudflatFactor > 0.02) {
        ...
        // Tidal pools in low-lying spots near the waterline
        float poolNear   = clamp(1.0 + depth / 20.0, 0.0, 1.0);
    ...
    // shadowBlend fades to 0 as depth goes positive (underwater), so no wasted work
    float shadowBlend = smoothstep(0.8, -0.3, depth);
    if (shadowBlend > 0.01) {
        const float E = 0.05;
    ...
    // ── Continuous depth-based water blend (replaces fixed-width teal band) ────
    //
    // Two curves driven by perturbed depth, no hard bands:
    //
    //   turbidity: how much sand-bottom is visible through the water column.
    //              High near the surface (0 m), zero at 5 m depth.
    //
    //   waterCover: how much the water tint dominates over the exposed surface.
    //              0% at depth = -1 m (exposed), 100% at depth = 7 m (deep).
    //
    // Wave perturbation on `depth` means each X-column transitions at a different
    // moment in time, producing the organic non-banded shoreline the eye expects.
    vec3 waterTint  = vec3(0.45, 0.74, 0.72);
    float turbidity = smoothstep(5.0, 0.0, depth);   // 0 = deep/opaque, 1 = surface/transparent
    vec3 sandBottom = mix(waterTint * 0.85, baseColor, 0.55);  // sand tinted by water
    vec3 shallowWater = mix(waterTint + waterTint * caust * 0.6, sandBottom, turbidity * 0.7);

    float waterCover = smoothstep(-1.0, 7.0, depth);
    baseColor = mix(baseColor, shallowWater, waterCover);

    // ── Foam reach (computed once, shared by wave body + swash + foam line) ───
