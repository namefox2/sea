...
void main() {
    ...
    if (shallowZone > 0.5) {
        ...
        // Minimal alpha: just darkens wet sand slightly, no visible color cast.
        waterAlpha = mix(0.01, 0.06, depth * depth) * edgeFade * waveActive;
        baseColor  = mix(baseColor, bottomColor * 0.78, waterAlpha);
    }
    ...
    // Swash extends over the full wave reach so foam+water cover where the teal film was.
    float swashFactor = smoothstep(waveReach * 1.2 + 1.5, 0.0, swashDist)
                      * smoothstep(-0.5, 0.0, distToWater) * waveActive;
    swashFactor *= swashFactor;
    // Far-end fade: suppress foam right at the waterline (ocean mesh handles that edge)
    // and also slightly toward the camera where perspective foreshortens the zone.
    float foamNear    = smoothstep(0.0, 0.28, dtwNorm);          // near waterline → 0
    float foamFarFade = 1.0 - smoothstep(0.68, 1.0, dtwNorm) * 0.45; // near camera → 55%
    swashFactor *= foamNear * foamFarFade;
    if (swashFactor > 0.001) {
        float shimmer  = bN(v_World.xz * 0.55 + vec2(u_Time * 0.07, -u_Time * 0.05)) * 0.40 + 0.60;
        vec3  swashRef = mix(mix(waterTint, wetSand, 0.45), u_Horizon * 0.50, wFres * 0.16);
        baseColor = mix(baseColor, swashRef * shimmer, swashFactor * 0.62);
        ...
        baseColor = mix(baseColor, vec3(0.96, 0.98, 1.00),
                        fineBubbles * swashFactor * 0.58 * (1.0 - lodFade * 0.6));
    }
