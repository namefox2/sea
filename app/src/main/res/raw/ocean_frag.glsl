...
uniform sampler2D   uMtnTex;    // mountain silhouette RGBA texture

// ── Scene constants — NEVER driven by tide / wave / wind ─────────────────────
const float HORIZON_Y  = 0.62;   // horizon line, absolutely fixed
const float MTN_ZONE_H = 0.20;   // mountain zone height above horizon (UV units)
const float MOON_X     = 0.72;
const float MOON_Y     = 0.820;
...
// Output: height displacement (wH) and 2-D screen-space gradient (wN).
// Used for SHADING ONLY — no boundary position is affected.
...
// ══════════════════════════════════════════════════════════════════════════════
// LAYER 1 — Sky
// Pure gradient from horizon to zenith. Daytime adds cloud FBM.
// ══════════════════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════════════════
// LAYER 2a — Stars (part of sky, composited before moon and mountains)
// ══════════════════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════════════════
// LAYER 2b — Moon / Sun disc  (fixed at MOON_X, MOON_Y — never moves)
// ══════════════════════════════════════════════════════════════════════════════
vec3 renderMoon(vec2 uv, vec3 base) {
    ...
    float glow = exp(-d * hR) * (uDark > 0.5 ? 0.68 : 0.65);
    ...
    float inten = uDark > 0.5 ? 1.18 : 1.85;
    float alpha = clamp(disc + glow * (1.0 - disc) + sbStr * (1.0 - disc), 0.0, 1.0);
    return mix(base, uSun * inten * limb, alpha);
}

// ══════════════════════════════════════════════════════════════════════════════
// LAYER 3 — Mountain  (PNG texture, absolutely fixed, tide/wave blind)
// Texture layout: y=0 (top) = mountain peak area, y=1 (bottom) = horizon base.
// Alpha channel: 1 = mountain body, 0 = sky.
// ══════════════════════════════════════════════════════════════════════════════
vec3 renderMountain(vec2 uv, vec3 skyCol) {
    float zTop = HORIZON_Y + MTN_ZONE_H;
    if (uv.y < HORIZON_Y || uv.y > zTop) return skyCol;

    // tex_v: 1.0 at horizon (base), 0.0 at top of mountain zone
    float tex_v = 1.0 - (uv.y - HORIZON_Y) / MTN_ZONE_H;
    vec4  samp  = texture2D(uMtnTex, vec2(uv.x, tex_v));

    // Slight moonlight brightening toward the ridge top
    float ridgeT = 1.0 - tex_v;
    vec3  mtnCol = uMtn * (0.82 + ridgeT * 0.22) + uSun * uDark * ridgeT * 0.05;
    return mix(skyCol, mtnCol, samp.a);
}

// ══════════════════════════════════════════════════════════════════════════════
// LAYER 4 — Ocean  (base water color — no specular here, that is Layer 5)
// depth 0 = near shore, 1 = horizon.
// ══════════════════════════════════════════════════════════════════════════════
vec3 renderOcean(vec2 uv, float shoreY, float wind, float wH, vec2 wN) {
    ...
    // Wave-modulated perceived depth → color shimmer
...
// ══════════════════════════════════════════════════════════════════════════════
// LAYER 5 — Reflection  (윤슬: specular moon/sun path on water surface)
// Decoupled from base ocean color so width/intensity can be tuned independently.
// ══════════════════════════════════════════════════════════════════════════════
vec3 renderReflection(vec2 uv, float shoreY, float wind, float wH, vec2 wN, vec3 base) {
    float waterH = HORIZON_Y - shoreY;
    float depth  = clamp((uv.y - shoreY) / max(0.001, waterH), 0.0, 1.0);

    vec3 norm    = normalize(vec3(wN.x * 0.85, 1.0, wN.y * 0.55));
    vec3 sunDir  = normalize(vec3((MOON_X - 0.5) * 0.90, 0.62, 0.60));
    vec3 viewDir = normalize(vec3(0.0, 0.45 + depth * 0.55, 1.0));
    vec3 halfV   = normalize(sunDir + viewDir);

    // Night: soft broad highlight; day: sharp pinpoint
    float sPow = uDark > 0.5 ? (60.0 + wind * 110.0) : (200.0 + wind * 400.0);
    float spec  = pow(max(0.0, dot(norm, halfV)), sPow);

    // Reflection column width proportional to wind strength
    float colW   = 0.26 + wind * 0.18;
    float sunPth = exp(-pow((uv.x - MOON_X) * uAspect * colW * 2.6, 2.0));
    spec *= (0.10 + 0.90 * sunPth) * (1.0 - (1.0 - depth) * 0.40);
    vec3 col = base + uSun * spec * (uDark > 0.5 ? 5.2 : 2.8);

    // Fine glitter sparkles along reflection band
    float gN    = noise(vec2(uv.x * 210.0, uv.y * 390.0 - uTime * 3.4));
    float gBand = exp(-pow((uv.x - MOON_X) * uAspect * (4.5 - wind * 1.2), 2.0));
    col += uSun * step(0.966, gN) * gBand * 3.2;
...
// ══════════════════════════════════════════════════════════════════════════════
// LAYER 6 — Foam  (whitecaps and shore break)
// Shore foam position is at shoreY — fixed per-frame, no wave wobble.
// ══════════════════════════════════════════════════════════════════════════════
vec3 renderFoam(vec2 uv, float shoreY, float wind, float wH, vec2 wN, vec3 base) {
    ...
    // Shore break at fixed waterline (shoreY never wobbles)
...
// ══════════════════════════════════════════════════════════════════════════════
// LAYER 7 — Tidal Flat  (exposed mudflat below shoreY)
// Only the HEIGHT of the exposure changes with tide; texture/detail stays fixed.
// ══════════════════════════════════════════════════════════════════════════════
vec3 renderTidalFlat(vec2 uv, float shoreY) {
    float wx = uv.x * uAspect;
    float ft = clamp(uv.y / max(0.001, shoreY), 0.0, 1.0);  // 0=bottom, 1=waterline
...
//
// Screen zones:
//   y >= HORIZON_Y              → Sky (+ stars + moon) + Mountain
//   shoreY <= y < HORIZON_Y     → Ocean + Reflection + Foam
//   y < shoreY                  → Tidal Flat
//
// Moving element:  only ocean surface shading (via Gerstner waves)
// Fixed elements:  horizon line, mountain silhouette, moon/stars position
// Tide changes:    only shoreY (how much flat is exposed)
// ══════════════════════════════════════════════════════════════════════════════
void main() {
    float wind   = uWind / 12.0;
    // shoreY = flat↔water boundary; moves with tide only
    float shoreY = max(0.002, (1.0 - uTide) * 0.300 - 0.010);

    vec3 col;

    if (vUV.y >= HORIZON_Y + 0.018) {
        // ── Pure sky zone ─────────────────────────────────────────────────────
        col = renderSky(vUV);
        ...
        // ── Horizon blend band (fixed, ~26 px wide on 1920-tall screen) ───────
        ...
        seaCol = renderReflection(vUV, shoreY, wind, wH, wN, seaCol);
        seaCol = renderFoam(vUV, shoreY, wind, wH, wN, seaCol);

        // Static foam burst right at the horizon (no wave wobble on the line)
        ...
        col = renderReflection(vUV, shoreY, wind, wH, wN, col);
        ...
        waterCol = renderReflection(vUV, shoreY, wind, wH, wN, waterCol);
